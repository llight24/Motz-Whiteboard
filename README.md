...
每个素材库都可以选择“复制副本”或“引用原文件”。复制模式适合长期收藏，原始文件移动或删除后库内副本仍可使用；引用模式不重复占用空间，适合已经整理好的大型本地素材目录。

素材可以放进多级分类，也能用标签、名称、来源和主色查找，并按添加时间、名称、分辨率或文件大小排序。
...
索引、键位与设置保存在每个素材库自己的 `.motz_data` 目录里（默认带隐藏属性，资源管理器里看不到）：`index.json` 放素材、分类、白板和排版索引，`keybindings.json` 放快捷键，`settings.json` 放当前库的导入方式与视图设置。它们跟着素材库一起搬家——把库目录拷到别的盘或别的机器，素材、索引、键位和设置都还在。

素材库列表、当前选中的库、窗口布局与缩略图缓存在 `%APPDATA%\MOTZ白板`：这些数据需要在本机上定位素材库，因此不随库移动。
